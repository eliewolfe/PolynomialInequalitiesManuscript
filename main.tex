...
Given a probability distribution over some random variables together with some hypothesis of causal structure, it is desirable to determine 
...
whether these two are compatible, i.e.~whether the hypothesized causal structure has the in principle ability to generate the given distribution, or more generally to determine such causal structures from the distribution only without a given hypothesis. This type of problem comes up in a wide variety of statistics application, from sussing out biological pathways to enabling machine learning \cite{pearl2009causality,spirtes2011causation,studeny2005probabilistic,koller2009probabilistic}. 

In the simplest case, the causal structure hypothesis consists of a DAG which only contains the observed variables as nodes. In this case, obtaining a verdict on the compatibility of the given distribution with the causal structure is simple: the compatibility holds if and only if the distribution is Markov with respect to the DAG, and there exists a variety of techniques to determine such compatible causal structures from the distribution only, i.e.~without having an \emph{a priori} hypothesis~\cite{pearl2009causality}.

A significantly more difficult case is when one considers the possibility that the causal structure hypothesis consists of a DAG with \tblue{latent} or \tblue{hidden} nodes, so that the set of observable variables is only a subset of the nodes of the DAG. This case occurs e.g.~in situations where one needs to deal with the possible presence of unobserved \tblue{confounders}, as in experiment design. While the conditional independence relations implied by the causal structure on the observable nodes are clearly still necessary conditions for compatibility of a given distribution with the causal structure hypothesis, these equations are generally no longer sufficient. Historically, this has first been observed by Bell in the context of the hidden variable problem of quantum physics~\cite{bell1964einstein}, who derived an inequality that any distribution compatible with the spacetime causal structure must satisfy, and found this inequality to be violated by distributions generated from suitably entangled quantum states\footnote{This incompatibility has subsequently become known as \emph{quantum nonlocality}. Although the term suggests the existence of nonlocal interactions which result in the actual causal structure being different from the hypothesized one, this interpretation is at odds with the fact that no nonlocal interactions have been observed in nature, and their presence would be unable to explain the observed absence of signalling and thereby require fine-tuning~\cite{WoodSpekkens}. A less problematic interpretation concludes instead the impossibility to model quantum physics in terms of the usual notions of ``classical'' probability theory.}. Subsequently, Pearl has derived another inequality, the \tblue{instrumental inequality}, for the purpose of witnessing the impossibility of modeling a three-variable distribution by the \emph{instrumental scenario} causal structure hypothesis. Steudel and Ay have derived inequalities which may witness the impossibility to model a given distribution on $n$ variables in terms of a causal structure without a common (hidden) ancestor for any subset of at least $c$ nodes, for any $n,c\in\mathbb{N}$. Motivated by Bell's inequality, many other Bell-type inequalities have since been applied in quantum physics~\cite{Brunner2013Bell}, but the foundational role of causal structure has only recently been appreciated~\cite{WoodSpekkens,fritz2012bell,pusey2014gdag,BeyondBellII}.
...
More generally, we can use Fine's theorem to show that applying the marginal problem to suitable inflation DAGs can reproduce \emph{all} Bell inequalities, for any one-common-cause Bell scenario, no matter how many parties or settings or possible outcomes. \purp{Tobias, check me on that last sentance?}
...
\purp{NOTE: term ``gedankenprobability" has been removed, cleanup required therefore. Also redundant/irredundant no longer defined. ~EW}
...
Constraints derived from non-broadcasting inflations are valid even in the GPT paradigm. Consequently the inequality in \cref{eq:polymonogamy}, which was derived from \cref{fig:TriDagSubA2B1C1}, is therefore a causal infeasibility criterion which holds for the Triangle scenario even when the latent nodes are allowed to be quantum states. This confirms our numerical computations, which indicated that~\eqref{eq:polymonogamy} does not have any quantum violations. The same is true for monogamy of correlations, per \cref{eq:monogomyofcorrelations}. Since the GHZ-type distribution~\cref{eq:ghzdistribution1} violates both of these inequalities, it is forbidden even per the relaxed GPT Triangle scenario, as was pointed out earlier. 
