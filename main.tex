...
In the literature on Bell inequalities, it has been noticed that the infeasibility with the Bell scenario DAG can sometimes be witnessed by only consider which (joint) outcomes have zero probability and which ones have nonzero probability. In other words, instead of considering the probabilities of an outcome, it may sufficient to only contemplate their \emph{possibility}. This has been originally noticed by Hardy, and hence such possibilistic are also known as \tblue{Hardy-type paradoxes}.

For example, suppose that we are in a marginal scenario where the pairwise joint distributions of three variables $A$, $B$ and $C$ are given, and these two variables are binary with values in $\{0,1\}$. Then we have the implication
\[
	(A=0,C=1) \Longrightarrow (A=0,B=0) \lor (B=1,C=1),
\]
and using the union bound on the probabilities, this translates into the inequality
\[
	P_{AC}(01) \leq P_{AB}(00) + P_{BC}(11),
\]
which is therefore a necessary condition for the marginal problem to have a solution. Applying this together with the inflation technique to the Triangle scenario results precisely in our earlier inequality.
...
\begin{align*}
	...
	\{A_2 B_2 C_2\}.
\end{align*}
Now a possibilistic constraint on this marginal problem consists of a logical implication with one joint outcome as the implicant and a disjunction of joint outcomes as the conclusion (implicate?). In the following, we explain how to generate \emph{all} such implication which are tight in the sense that their right-hand sides are minimal. So let us fix some joint outcome as the left-hand side, taking
\begin{equation}
	\label{eq:lhshardy}
	(A_2 = 1, B_2 = 1, C_2 = 1)
\end{equation}
for the sake of concreteness. Then assuming these values, the right-hand side of the implication needs to be a conjunction of joint outcomes which is such that for any deterministic assignment of values to the variables which extends~\cref{eq:lhshardy}, also one of the outcomes on the right-hand side occurs. To determine what the possibilities for choosing this right-hand side are, it helps to draw the hypergraph in which the set of nodes is the disjoint union of all the outcomes over all the contexts other than~\cref{eq:lhshardy}, and the hyperedges correspond to complete deterministic assignments of values to all variables extending~\cref{eq:lhshardy}, and such a hyperedge contains precisely those nodes that are the marginal outcomes to which it restricts. Then the possible right-hand sides of the implications are precisely the \tblue{transversals} of this hypergraph, i.e.~the sets of nodes which have the property that they intersect every hyperedge in at least one node. In order to get implications which are as tight as possible, it is sufficient to enumerate only the \tblue{minimal transversals}. Doing so is a well-studied problem for which various software suites exist. This enumeration, even for every possible left-hand side of the implication, is computationally much more tractable than the linear quantifier elimination.

In our example, it is not hard to check that the right-hand side of
\[
	(A_2 = 1, B_2 = 1, C_2 = 1) \Longrightarrow (A_1 = 0, B_1 = 0, A_1 = 0) \lor (A_1 = 1, B_2 = 1, C_2 = 1) \lor (A_2 = 1, B_1 = 1, C_2 = 1) \lor (A_2 = 1, B_2 = 1, C_1 = 1)
\]
...
The process of linear quantifier elimination, while orders of magnitude more computationally amenable than its nonlinear variant, is nevertheless nontrivial. When the number of observable random variables in the inflation DAG is too large, it quickly happens that even the most advanced linear quantifier elimination algorithms become too slow for practical application. To this end we have developed a strategy that identifies only one particular class of inequalities for the marginal problem, but does so very efficiently. Using the inflation DAG technique, we can subsequently convert these into polynomial inequalities for causal inference.

As it currently stands, the TSEM method applies to binary variables only, say with values in $\{0,1\}$. This is what we assume from now on.
...
\end{align}
Note that we treat statistical events such as random variables yielding particular outcomes as identically logical propositions. The TEM can also be supplemented with certain ``givens", which we take to be known-true propositions on the left-hand-side. These ``givens" can then be interspersed throughout the right-hand side while still yielding a valid tautology. Thus
...
TSEM inequalities can be used as necessary constraints for the solvability of the marginal problem. As such, we can combine them with the inflation DAG technique in order to obtain polynomial inequalities for causal inference. For example the TSEM inequality
\begin{align}
...
In fact, many of the polynomial inequalities of this paper are TSEM inequalities; for example, the Triangle scenario inequality \cref{eq:trifancyunmapped} corresponds to the logical TSEM
\begin{align}
...
\end{align}
%and the CHSH inequality in \cref{eq:bellypolyunmapped} follows from the logical TSEM
%\begin{align}
...
Generating TSEM inequalities is computationally much easier than performing the linear quantifier elimination, and we now outline how to do this. First, by symmetry we may assume that the left-hand side of the tautology contains only unnegated variables. Thus formally, the tautology consists of
\[
	\bm{L} \quad\Longrightarrow\quad (\bm{A}_1,\bm{B}_1)\lor\ldots\lor(\bm{A}_n,\bm{B}_n),
\]
where each of these sets is a simplex in the simplicial complex that describes the marginal problem. In logical term, 


%A practical alternative to \cref{sec:mainalgorithm}, then, is the four-step process
%\begin{compactenum}
...
%\end{compactenum}
Restricting one's search to TSEM inequalities makes deriving causal infeasibility criteria tractable even for large DAGs, but a consequence, however, is that only one class of possible inequalities is being considered.
...
\begin{align}
    ...
    \An[G_{\setminus\bm{Z}}]{\bm{X}}& \text{if } \An[G_{\setminus\bm{Z}}]{\bm{X}}\text{ and }\An[G]{\setminus\bm{Z}}\text{ are disjoint}\\
    \An[G_{\setminus\bm{Z}}]{\bm{X}} \bigcup \An[G]{\setminus\bm{Z}} & \text{if } \An[G_{\setminus\bm{Z}}]{\bm{X}}\text{ and } \An[G]{\setminus\bm{Z}}\text{ overlap.}
    \end{cases}
\end{align}
...
For any two nodes sets $\bm{X}$ and $\bm{Y}$ such that $\bm{X}\sim\bm{Y}$, there exists an \tblue{inflationary isomorphism} which related the two sets such that nodes in $\bm{X}$ are mapped bijectively to nodes in $\bm{Y}$ such that nodes are mapped to copies, i.e. $A_i\leftrightarrow A_i$ or $A_i\leftrightarrow A_j$ but never $A_i\leftrightarrow B_i$ nor  $A_i\leftrightarrow B_j$. There may exist multiple inflationary isomorphisms, for example the sets $\brackets{A_1 A_2 B_1}$ and $\brackets{A_1 A_2 B_2}$ can be related by either 
\begin{align}
    \begin{pmatrix}
...
Constraints of this form be used to supplement the marginal problem without requiring nonlinear quantifier elimination, but they will not have \emph{impact} unless used with conditional independence relations and hence nonlinear quantifier elimination. \purp{Find a good way to explain why not? No impact of projected polytope because...}
