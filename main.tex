...
Constraints of the above form be used to supplement the marginal problem without requiring nonlinear quantifier elimination, but they will not have \emph{impact} unless used together with conditional independence relations and hence nonlinear quantifier elimination. \purp{Find a good way to explain why not? No impact of projected polytope because...}
...
\purp{T: ESP is supposed to be good when the number of vertices far exceeds the number of facets. Judging by the~\cref{fig:Tri222} situation, this is not a situation that we should expect. Is there still any chance that ESP is useful at all, or can this stuff be removed?}
...
\purp{T: I think that we should omit this entirely except for the final sentence, which can be moved to the main text. Any Mathematica code is too hard for the uninitiated reader (such as myself) to understand.}
...
\clearpage
\includepdf[pages=-,scale=.95]{64InequalitiesForTobias.pdf}
