...
\end{align}
Note that this implies that certain subgraphs of $G'$ are identical.  Specifically, 

Furthermore, For $C'$ to be an inflation of $C$, the parameters $\mathcal{P'}(G')$ must be related to the parameters $\mathcal{P}(G)$ as follows: 
...
Now suppose one considers the set $\mathcal{C}_G$ of all causal models associated with a DAG $G$, which is to say the set of pairs $(G, \mathcal{P}(G))$ wherein $\mathcal{P}(G)$ are a set of valid conditional probabilities, 
\begin{align}
\mathcal{C}_G \equiv \{ C: \SmallNamedFunction{DAG}{C} =G\}.
\end{align}
Let the set of causal models $\mathcal{C}'_{G'}$ be the image of $\mathcal{C}_G$ under some nontrivial inflation map.  We note that $\mathcal{C}'_{G'}$ is {\em not} the set of all causal models associated with $G'$.  Rather, there is a nontrivial constraint on the parameters.  Given Eq.~\eqref{eq:funcdependences}, we see that for any node of $G$ for which $G'$ constains multiple copies, the conditional probability distributions associated to the latter are the same.  That is, if $A_i \sim A_j$, then 
\begin{align}
\pdf{A_i| {\rm Pa}_{G'}(A_i)}=\pdf{A_j|{\rm Pa}_{G'}(A_j)}.
\end{align}



 only if $G' \in \SmallNamedFunction{Inflations}{G}$ and 
%A set of causal models $\mathcal{C}'_{G'}$ is the image of $\mathcal{C}_G$ under some inflation map only if $G' \in \SmallNamedFunction{Inflations}{G}$ and 
