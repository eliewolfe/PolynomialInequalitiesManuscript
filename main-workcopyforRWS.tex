...
When a causal model constitutes a hypothesis for explaining a probability distribution over a set of observed variables, then it is natural to distinguish the nodes in the DAG that correspond to observed variables and the rest, which are termed latent variables.  
 In our graphical depictions of the DAG, we follow the convention of representing latent nodes by circles, and observable nodes by triangles \cite{pusey2014gdag}.
 \color{black}
