...
\begin{lemma}
%Suppose the original causal inference problem is to determine the consistency of a probability distribution over a set $\bm{O}$ of observed variables, denoted $\pdf{\bm{O}}$ with a causal hypothesis $H_{G,S}$.  
Suppose the original causal inference problem takes as its inputs  a probability distribution over a set $\bm{O}$ of observed variables, denoted $\pdf{\bm{O}}$, and a causal hypothesis $H_{G,S}$, and seeks to determine whether these are consistent.  

We now define a new causal inference problem at the level of the inflation DAG.  The causal hypothesis is an inflation of the causal hypothesis $H_{G,S}$. 
observational input 

Let $H_{G',S'}$ be a causal hypothesis that is the image under inflation of the causal hypothesis $H_{G,S}$. 

 Let $\{ P(\bm{U}_i) : \bm{U}_i \in \SmallNamedFunction{PreInjectableSets}{G'} \}$ 
