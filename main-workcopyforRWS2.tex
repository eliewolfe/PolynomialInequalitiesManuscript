...
The joint distribution over the observed variables is obtained from the joint distribution over all variables by marginalization over the latent variables $\pdf{\SmallNamedFunction{ObservedNodes}{G}} =  \sum_{\{X :X \in\SmallNamedFunction{LatentNodes}{G}\}} \pdf{\SmallNamedFunction{Nodes}{G}}$.
...
We denote  the set of {\em all} parameter values consistent with $G$ by $S_{\rm full}$, so that $H_{G,{S_{\rm full}}}$ is simply the causal hypothesis that the DAG is $G$, that is, the hypothesis that there exists {\em some} parameter values supplementing $G$ such that the observed data can be explained by the resulting causal model. 


With these notions in hand, we can give a broad definition of causal inference.  It is useful to characterize any algorithm for causal inference as one that takes two inputs:
\begin{itemize}
\item an {\em observational input}, which is a specification of some feature of observed statistical data
...
\item a {\em causal input}, which is a specification of some causal hypothesis
\end{itemize}
and that outputs some sort of verdict about their compatibility.

The most informative sort of observational input is a specification of the joint distributions over all of the observed variables.  The observational input in real-world applications necessarily falls short of this ideal because one only ever obtains a {\em finite sample} of the joint distribution.  Moreover, even if one sets aside finite-sample concerns and imagines the input as infinite-run statistics, there are other ways in which the observational input to a causal inference problem might be less informative than a specification of the joint distribution.  For instance, one might merely have a description of the conditional independence relations that hold among the observed variables.  Or one might merely have a specification of the marginals of the joint distribution for certain subsets of the observed variables.  Indeed, the latter sort of restriction on the observational input will be relevant for our inflation DAG technique.  

The most fine-grained specification of the causal input is one wherein the causal hypothesis specifies a specific DAG as well as a complete specification of all the parameters supplementing the DAG.  The causal inference problem that is perhaps most widely studied is one wherein the causal hypothesis just concerns the DAG.  Between these are causal inference problems that explore more refined causal hypotheses, incorporating constraints on how  certain nodes in the DAG functionally depend on their parents.  An example is the assumption of an additive noise model for certain causal influences: if observed variable $Y$ has an observed variable $X$ as a parent and also a latent variable $U$, then the noise is deemed additive if $Y=\alpha X + \beta U$ for some scalars $\alpha$ and $\beta$ [provide references].  More general constraints on the causal dependences in the model have also been explored [provide references].  (Note that constraints on the probability distributions over the latent variables can be understood as a kind of constraint on the causal dependences.)
...
\subsection{Inflation: a tool for causal inference}

We now introduce the notion of \tblue{the inflation of a causal model}.  We describe what inflation means for the causal structure and the parameters in turn. 
...
\mathcal{P'}(G') = \SmallNamedFunction{Inflation}{\mathcal{P}(G)} \quad \text{iff}\quad \forall A_i \in \SmallNamedFunction{Nodes}{G'}:\; \pdf{A_i| \Pa[G']{A_i}}=\pdf{A|\Pa[G]{A}},
\end{align}
...
To sum up then, inflation is a mapping to a new causal structure wherein each given variable in the original causal structure may have counterparts in the inflation and {\em where the causal mechanisms in the inflation DAG are given by the corresponding 
%autonomous 
causal mechanisms in the original DAG.}  
...
Finally, we can begin to spell out how the inflation DAG technique is useful for causal inference.  First, we emphasize the broad notion of causal inference that we have in mind here. 
...
To see how inflation is relevant for causal inference, we must explain how the distributions that can be achieved in the inflation model are constrained by those in the original model.
...
\end{align}
Furthermore, because each injectable set of variables $\bm{U}_i$ satisfies Eq.~\eqref{eq:coincidingdistrodef}, it follows that such joint distributions in the inflation model can be expressed in terms of distributions defined on the original causal model,
\begin{align}\label{eq:preinjfactor}
...
\begin{lemma}
The causal hypothesis $H_{G,S}$ is consistent with statistical data $\pdf{\SmallNamedFunction{ObservedNodes}{G}}$  if and only if the causal hypothesis $H_{G',S'}$ is consistent with the following constraints on the distribution over the observed variables in $G'$: 
\begin{align}
\forall \bm{U} \in \SmallNamedFunction{InjectableSets}{\SmallNamedFunction{ObservedNodes}{G'}}: \pdf[G']{\bm{U}} = \pdf{\subsim{\bm{U}}}.
\end{align}
\end{lemma}

We see, therefore, that a causal inference problem at the level of the original DAG defines a causal inference problem at the level of the inflated DAG.


%Let the set of observed variables be denoted by $\bm{O}$, and consider a given joint distribution over $\bm{O}$, denoted $\pdf{\bm{O}}$.  A causal model is only a candidate for a causal explanation of  $\pdf{\bm{O}}$ if it posits a DAG $G$ satisfying $\SmallNamedFunction{ObservedNodes}{G}=\bm{O}$, so we assume this henceforth.  
%The causal hypothesis $H_{G,S}$ is consistent with data $\pdf{\bm{O}} if and only if there exists a causal model satisfying the constraints of the hypothesis, $C \in H_{G,S}$, such that $\pdf[C]{\SmallNamedFunction{ObservedNodes}{G}} = \pdf{\bm{O}}$. 
%\begin{lemma}
%The causal hypothesis $H_{G,S}$ is consistent with a distribution $\pdf{\bm{O}}$ over the observed variables in $G$  if and only if the causal hypothesis $H_{G',S'}$ is consistent with the following constraints on the distribution over the observed variables in $G'$: $\forall \bm{U} \in \SmallNamedFunction{InjectableSets}{\SmallNamedFunction{ObservedNodes}{G'}} \cap : \pdf[G']{\bm{U}} = \pdf{\subsim{\bm{U}}}$.
%\end{lemma}


%If the DAG $G$ is inflated to the DAG $G'$, then we denote the image of $\SmallNamedFunction{ObservedNodes}{G}$ under the inflation by $\SmallNamedFunction{ObservedNodes}{G'}$.  [....]
