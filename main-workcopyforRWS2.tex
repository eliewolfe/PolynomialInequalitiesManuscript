...
\end{align}
The causal hypothesis is the set of causal models associated to the DAG of the triangle scenario, depicted in Fig.~\ref{}.  The problem is to decide whether the observational data is consistent with this causal hypothesis.
...
\end{align}
The question is whether such constraints are consistent with the causal hypothesis that the DAG is that of Fig.~\ref{}, where the parameters supplementing the DAG are arbitrary, except for the constraint that the latent variables $Y_1$ and $Y_2$ are identically distributed. 
...
p_{\text{PR}}\parens{a b |x y}=\begin{cases}\tfrac{1}{2}&\text{if }\; \SmallNamedFunction[2]{mod}{a+b}=x\cdot y, \\ 0&\text{otherwise}.\end{cases}
\end{split}\end{align}
We begin by recognizing that $\{A_1 B_1 X_1 Y_1\}$, $\{A_2 B_1 X_2 Y_1\}$, $\{A_1 B_2 X_1 Y_2\}$, and $\{A_2 B_2 X_2 Y_2\}$ are all injectable sets, and that $X_1$, $X_2$, $Y_1$, and $Y_2$ are all independent variables.
% $P_{\text{PR}}$ is given as a conditional distribution because $X$ and $Y$ are settings variables.
...
\begin{comment}
Let $\SmallNamedFunction{Inflation}{\bm{O}}$ denote the image of $\bm{O}$ under the $G \to G'$ inflation.  %The observational input of the new causal inference problem is specified in terms of the observational input of the original causal inference problem as follows.  
%constraints on the joint distribution $\pdf{\SmallNamedFunction{Inflation}{\bm{O}}}$, namely, that the marginals of this distribution on the pre-injectable sets be equal to the corresponding marginals of $\pdf{\bm{O}}$, 
Define the observationally constrained subsets of $\SmallNamedFunction{Inflation}{\bm{O}}$ in the new causal inference problem, denoted $\SmallNamedFunction{ObsConstrainedSubsets}{\SmallNamedFunction{Inflation}{\bm{O}}}$ by
\begin{align}
\bm{U} \in \SmallNamedFunction{ObsConstrainedSubsets}{\bm{O}} \quad \text{iff} \quad 
\bm{U}_i \in \SmallNamedFunction{PreInjectableSets}{\SmallNamedFunction{Inflation}{\bm{O}}} 
\end{align}
% \forall \bm{U}_i \in \SmallNamedFunction{PreInjectableSets}{\SmallNamedFunction{Inflation}{\bm{O}}} : 
 
We now define a new causal inference problem at the level of the inflation DAG.  The causal hypothesis in this case is the inflation of the causal hypothesis $H_{G,S}$ and the observational data 
observational input 
