...
\begin{comment}
%\begin{align}\label{eq:triintegration}
%p&\parens{A\mathopen{=}a,B\mathopen{=}b,C\mathopen{=}c}
%\\\nonumber&=\int\limits_{\mathclap{\substack{\lambda_{AB}\in \Omega_{AB}\\\lambda_{BC}\in \Omega_{BC}\\\lambda_{AC}\in \Omega_{AC}}}}\!\!\begin{pmatrix}\p{A\mathopen{=}a|\lambda_{AB},\lambda_{AC}} \p{B\mathopen{=}b|\lambda_{AB},\lambda_{BC}}\times\\ \p{C\mathopen{=}c|\lambda_{AC},\lambda_{BC}}\p{\lambda_{AB}}\p{\lambda_{BC}} \p{\lambda_{AC}}\end{pmatrix}.
%\end{align}
Consider now the following tautology
\begin{taut}\label{taut:Spekkens}The following is tautologically true:
\begin{align*}
\text{if }\;\; \p{a_{|\_1}}&\leq\p{{b'}_{|1\_}},\\
\text{and }\;\; \p{b_{|\_2}}&\leq\p{{c'}_{|2\_}},\\
\text{and }\;\; \p{c_{|\_3}}&\leq\p{{a'}_{|3\_}},\\
\text{then }\;\; \p{a_{|11}}\p{b_{|22}}\p{c_{|33}}&\leq\p{{a'}_{|31}}\p{{b'}_{|12}}\p{{c'}_{|23}}.
\end{align*}
\end{taut}
%\begin{align}
%{\rm if  }\;\; &\lambda_{AB} : \bracks*{a|\lambda_{AB}} {\scriptstyle \implies} \bracks*{\nb|\lambda_{AB}},\label{eq:AimpBalt}\\
%{\rm and }\;\; &\lambda_{BC} : \bracks*{b|\lambda_{BC}} {\scriptstyle \implies} \bracks*{\nc|\lambda_{BC}},\label{eq:BimpCalt}\\
%{\rm and }\;\; &\lambda_{AC} : \bracks*{c|\lambda_{AC}} {\scriptstyle \implies} \bracks*{\na|\lambda_{AC}},\label{eq:CimpAalt}\\
%{\rm then }\;\; &\bracks*{a|\lambda_{AB} , b|\lambda_{BC} , c|\lambda_{AC}} {\scriptstyle \implies} \bracks*{\na \nb %\nc|\lambda_{AB}\lambda_{BC}\lambda_{AC}} .
%\end{align}
If the first three conditions of  \cref{taut:Spekkens} are effectively verified for \emph{all} latent variables (in that no exception is ever noticed) then the corresponding conclusion holds true in general. 

When we integrate the final inequality in \cref{taut:Spekkens} we find that the causal structure treats the two sides of the inequality quite differently. The left-hand-side factors, namely
\begin{align*}
\int_{\setlambda}\,
\begin{pmatrix}
\p{a|\lambda_{AB}^{(1)}\lambda_{AC}^{(1)}}
\\\times \p{b|\lambda_{AB}^{(2)}\lambda_{BC}^{(2)}}
\\\times \p{c|\lambda_{BC}^{(3)}\lambda_{AC}^{(3)}}
...
\end{align*}
but the right-hand-side does not, namely
\begin{align*}
\int_{\setlambda}\,
\begin{pmatrix}
\p{a'|\lambda_{AB}^{(1)}\lambda_{AC}^{(3)}}
\\\times \p{b'|\lambda_{AB}^{(1)}\lambda_{BC}^{(2)}}
\\\times \p{c'|\lambda_{BC}^{(2)}\lambda_{AC}^{(3)}}
\\\times \p{\lambda_{AB}^{(1)}}\p{\lambda_{BC}^{(1)}} \p{\lambda_{AC}^{(1)}}
\\\times \p{\lambda_{AB}^{(2)}}\p{\lambda_{BC}^{(2)}} \p{\lambda_{AC}^{(2)}}
\\\times \p{\lambda_{AB}^{(3)}}\p{\lambda_{BC}^{(3)}} \p{\lambda_{AC}^{(3)}}
\end{pmatrix}=\p{a' b' c'}.
...
\begin{comment}
\begin{proof}
First we convert the deterministic relations supposed by \cref{eq:AimpNotBsource,eq:BimpNotCsource,eq:CimpNotAsource} into contrapositive statements; they state that $a {\scriptstyle \implies} \nbp$, $b {\scriptstyle \implies} \ncp$, and $c {\scriptstyle \implies} \nap$. Now, if $X\cramp{=}x {\scriptstyle \implies} Y\cramp{=}y$ then whenever the common latent parent of $X$ and $Y$ takes on a value which give rise to $X\cramp{=}x$ we can be assured that $Y$ also takes on the value of $y$ regardless of values of any other of $Y$'s parent variables. As such, \cref{eq:AimpNotBsource,eq:BimpNotCsource,eq:CimpNotAsource} further imply that
\begin{align}
&\forall_{\lambda_{AB} }\; \p{b'|\lambda_{AB},\lambda_{BC}}\geq \p{a|\lambda_{AB}}\\
&\forall_{\lambda_{BC} }\; \p{c'|\lambda_{BC},\lambda_{AC}}\geq \p{b|\lambda_{BC}}\\
&\forall_{\lambda_{AC} }\; \p{a'|\lambda_{AC},\lambda_{AB}}\geq \p{c|\lambda_{AC}}
\end{align}
It then follows that
\begin{align}
\nonumber p&\parens{a}\p{b}\p{c}
...
\\ &=\iiint\limits_{\mathclap{\lambda_{AB}\lambda_{BC}\lambda_{AC}}}\,
%\limits_{\mathclap{\substack{\lambda_{AB}\in \Omega_{AB}\left[\_\_\right]\\\lambda_{BC}\in \Omega_{BC}\left[\_\_\right]\\\lambda_{AC}\in \Omega_{AC}\left[\_\_\right]}}}
\begin{pmatrix}\p{a|\lambda_{AB}} \p{b|\lambda_{BC}}\p{c|\lambda_{AC}}\times\\ \p{\lambda_{AB}}\p{\lambda_{BC}} \p{\lambda_{AC}}\end{pmatrix}
...
%\!\!\!\begin{pmatrix}\p{\nbp|\lambda_{AB},\lambda_{BC}} \p{\ncp|\lambda_{BC},\lambda_{AC}}\times\\ \p{\nap|\lambda_{AC},\lambda_{AB}}\p{\lambda_{AB}}\p{\lambda_{BC}} \p{\lambda_{AC}}\end{pmatrix}
\\\nonumber&=\p{a' b' c'}
\end{align}
...
\purp{New text only PRIOR to this point.}
